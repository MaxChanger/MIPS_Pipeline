...
class PipelineTopTests(c: Top) extends PeekPokeTester(c) {
    ...
    def RunCpu (defcycle : Int, filename : File, test_num : Int) = 
    {
       ...
       // {
          ...
          //poke(c.io.test_dm_wr, 0)
            ...
            while(lastflag<2 /*&& cycles< 31000  peek(c.io.MEM_WB_inst) != (inst_last)*/) 
            {
 ...
 //       poke(c.io.rf_19_reset,1)
		...
		// print(s"PredictAccuarcy: ${peek(c.io.BranchPrediction_Branch_success)} / ${peek(c.io.BranchPrediction_Branch_number)}  \n\n")
        // printf("PredictAccuarcy:%.6f \n",a.toFloat / b.toFloat)
